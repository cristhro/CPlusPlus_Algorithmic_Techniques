...
long long parentesis(vector<char> const& X) {
	...
	Matriz<long long>A(n , n);
	Matriz<long long>B(n , n);
	Matriz<long long>C(n , n);
	...
	for (int d = 1; d < n - 1; d++)
	{
		...
		for (int i = 1; i < n-d; i++)
		{
			...
			for (int k = i ; k < j ; k++)
			{
				A[i][j] += (A[i][k] * C[k + 1][j] + B[i][k] * C[k + 1][j] + C[i][k] * A[k + 1][j]) % 1000000007;
				B[i][j] += (A[i][k] * A[k + 1][j] + A[i][k] * B[k + 1][j] + B[i][k] * B[k + 1][j]) % 1000000007;
				C[i][j] += (B[i][k] * A[k + 1][j] + C[i][k] * B[k + 1][j] + C[i][k] * C[k + 1][j]) % 1000000007;
			}
		}
	}
...
bool resuelveCaso() {
	...
	long long solucion =  parentesis(X);

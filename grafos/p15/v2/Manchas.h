...
public:
	/* constructor */
	Manchas(int rows, int cols) : num_max_mancha(0),G(rows*cols)
	{
		m_cols = cols;
		m_rows = rows;
	...
	void pon_Pixel(int f, int c) {
		...
		ComponentesConexas compConx(G);
		
		int tam = compConx.size(marcas[f][c]);
...
private:
	...
	void dfs_mancha(){
		...
		for (size_t i = 0; i < m_rows; i++)
		{
			...
			{
				marcas[i][j] = e;    //inicializamos marcas
				e++;
			}
	...
	void num_Connected(int f, int c, int &cont) {
		...
		if (connected_toWest(f, c - 1)) {
			G.ponArista(marcas[f][c], marcas[f][c-1]);
			cont++;
		...
		if (connected_toNorth(f - 1, c)){
			G.ponArista(marcas[f][c], marcas[f- 1][c ]);
